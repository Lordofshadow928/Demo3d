...
public class RotateByDrag : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
{
    ...
    private void Update()
    {
        if(isDragging)
        {
            player.localRotation = Quaternion.Lerp(player.localRotation, desiredPlayerRotation, Time.deltaTime * sensitivity);
            cameraHolder.localRotation = Quaternion.Lerp(cameraHolder.localRotation, desiredCameraRotation, Time.deltaTime * sensitivity);
        }
    }
#endif
}
